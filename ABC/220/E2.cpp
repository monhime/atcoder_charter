...
void sub_tree_cunt(int node, int parent, int depth, std::vector<ll> &Sub,
                   std::vector<ll> &Ans, std::vector<std::vector<int>> &graph) {
...
void dfs(int node, int parent, int n, std::vector<ll> &Ans,
         std::vector<ll> &Sub, std::vector<std::vector<int>> &graph) {
  for (auto xnode : graph[node]) {
    if (xnode == parent) {
      continue;
    }
