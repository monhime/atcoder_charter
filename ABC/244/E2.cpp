...
int main() {
  ...
  t--;
  x--;
  std::vector<std::pair<int, int>> graph;  //空の配列n個分
  for (int i = 0; i < m; i++) {
    int a, b;
    std::cin >> a >> b;
    graph.emplace_back(std::pair<int, int>(a - 1, b - 1));
  }
  ...
  for (int i = 1; i <= k; i++) {
    for (auto e : graph) {
      ...
      dp_tmp[node].first += (node == x ? dp[xnode].second : dp[xnode].first);
      dp_tmp[node].second += (node == x ? dp[xnode].first : dp[xnode].second);
      dp_tmp[node].first %= MOD;
      dp_tmp[node].second %= MOD;
    }
    dp = dp_tmp;
