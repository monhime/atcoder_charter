...
int main(){
    ...
    int n;
    io.read(n);
    vector<vector<Pill>> graph(n);//空の配列n個分
    REP(i, n - 1) {
        int a, b;
        ll c;
        io.read(a, b, c);
        a -= 1;
        b -= 1;
        graph[a].emplace_back(Pill(b, c));
        graph[b].emplace_back(Pill(a, c));
    }
    ...
    queue<int> que;
    que.emplace(k);
    vll Kyori(n, -1ll); //-1は未訪問
    Kyori[k] = 0ll;
    while (!que.empty()) {
        int node = que.front(); que.pop();
        for (auto &xnode : graph[node]) {
            if (Kyori[xnode.first] >= 0ll) {
                continue;
            }
            Kyori[xnode.first] = Kyori[node] + xnode.second;
            que.emplace(xnode.first);
        }
    }
